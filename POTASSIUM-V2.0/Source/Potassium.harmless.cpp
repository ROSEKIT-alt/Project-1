...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR lpCmd, int nShow) {
   if (MessageBoxW(NULL, L"Run Malware?", L"Potassium.exe (HARMLESS)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
       ExitProcess(0);
   } else {
       if (MessageBoxW(NULL, L"Are you sure?", L"Potassium.exe - LAST WARNING! (HARMLESS)", MB_YESNO | MB_ICONEXCLAMATION) == IDNO) {
           ExitProcess(0);
       } else {
